...
"""
Eric Qwen-Image Spectrum Accelerator node.

Adds Spectrum (training-free spectral diffusion feature forecasting) support
to the Qwen-Image generation pipeline.  Wire this node between the Image
Loader and any generation node (Generate / Multi-Stage / UltraGen).

When enabled, the transformer's block loop is skipped on predicted (cached)
steps, yielding a ~3–5× wall-clock speedup at ≥20 steps with minimal
quality loss.
...
class EricQwenImageSpectrum:
    """
    Spectrum Accelerator for Qwen-Image generation pipeline —
    training-free diffusion sampling speedup.
    ...
    DESCRIPTION = (
        "Spectrum Accelerator for Qwen-Image generation — training-free ~3–5× "
        "diffusion sampling speedup. Predicts transformer outputs on skipped "
        "steps using Chebyshev polynomial regression instead of running all "
        "transformer blocks. Wire between Image Loader and any generation node."
    )
